...
export default function Home(){
  ...
  const visible=useMemo(()=>schools.filter(s=>{const visit=visits[s.key];const days=ago(visit?.lastVisitedAt);const activityMatch=activityFilter==="all"||(activityFilter==="yesterday"&&days===1)||(activityFilter==="7days"&&days!==null&&days<=7)||(activityFilter==="month"&&days!==null&&days<=30)||(activityFilter==="notes"&&Boolean(visit?.notes?.trim()))||(activityFilter==="incidents"&&Boolean(visit?.incident));return(kind==="all"||s.kind===kind)&&(!query||s.search.includes(query.toLowerCase()))&&(visitFilter==="all"||(visitFilter==="visited"?visit?.visited:!visit?.visited))&&activityMatch}),[schools,kind,query,visitFilter,activityFilter,visits]);
  const open=(s:School)=>{setSelected(s);setDraft(visits[s.key]||{visited:false,lastVisitedAt:"",notes:"",incident:false,incidentNotes:""})};
  ...
  const save=async()=>{if(!user||!APPROVED_EMAILS.has(user.email?.toLowerCase()||"")||!selected)return;setSaving(true);await setDoc(doc(db,"sharedVisits",selected.key),{...draft,schoolName:selected.name,schoolId:selected.id,updatedBy:user.email,updatedAt:serverTimestamp()},{merge:true});setSaving(false);setSelected(null)};
  ...
  return <main className={`map-app ${sidebarHidden?"sidebar-hidden":""}`}>
    <aside className={`map-sidebar ${menu?"open":""}`}>
      ...
      <div className="filter-title"><span>Activity</span></div><div className="activity-grid">{([['all','Any time'],['yesterday','Yesterday'],['7days','Last 7 days'],['month','Last 30 days'],['notes','Has notes'],['incidents','Incidents']] as const).map(([value,label])=><button key={value} className={activityFilter===value?"active":""} onClick={()=>setActivityFilter(value)}>{label}</button>)}</div>
