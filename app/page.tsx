...
const classify = (r:Record<string,string>):Kind => {
  ...
  if (/therapy|^t$/.test(text)) return "therapy";
...
export default function Home(){
  ...
  const [draft,setDraft]=useState<Visit>({visited:false,lastVisitedAt:"",notes:"",sublocatorName:"",sublocatorPhone:"",sublocatorEmail:"",sublocatorRating:0,incident:false,incidentNotes:""}); const [saving,setSaving]=useState(false); const [menu,setMenu]=useState(false); const [sidebarHidden,setSidebarHidden]=useState(false);
  ...
  const open=(s:School)=>{setSelected(s);setDraft({...visits[s.key]||{visited:false,lastVisitedAt:"",notes:"",sublocatorName:"",sublocatorPhone:"",sublocatorEmail:"",sublocatorRating:0},incident:Boolean(incidents[s.key]?.active),incidentNotes:incidents[s.key]?.notes||""})};
  ...
  useEffect(()=>{if(!mapNode.current||!schools.length)return; let cancelled=false; (async()=>{
    ...
    const labels={elementary:"E",middle:"M",high:"H",k8:"K",therapy:"T",other:"S"};
  ...
  return <main className={`map-app ${sidebarHidden?"sidebar-hidden":""}`}>
    <aside className={`map-sidebar ${menu?"open":""}`}>
      ...
      <div className="filter-grid">{([['all','All'],['elementary','Elementary'],['middle','Middle'],['high','High'],['k8','K–8'],['therapy','Therapy Schools'],['other','Other']] as const).map(([k,n])=><button key={k} className={kind===k?"active":""} onClick={()=>setKind(k)}>{n}</button>)}</div>
